/**
 * ENGINE: HIGH-THROUGHPUT OFFLINE SCANNER
 * ------------------------------------------------
 * * CORE ARCHITECTURE:
 * - Context-Aware: Strictly requires `gate` and `eventId` params to prevent cross-event fraud.
 * - Offline-First: Network drops seamlessly route payloads to Zustand `syncStore.ts`.
 * - Rapid-Fire: Camera reset cooldown optimized to 800ms for high-volume entry gates.
 * - Anti Pass-Back: Toggles between 'IN' and 'OUT' modes to reverse ticket states.
 * * HARDWARE DEPENDENCIES:
 * - expo-camera: Requires camera permissions on mount.
 * - expo-audio: Synchronous native audio triggers (success/error) for "no-look" scanning.
 * - Device Vibration: Haptic feedback for accessibility in loud environments.
 * * @critical Do not modify the scan_type payload casing without updating the DB RPC.
 */
...
export default function ScannerScreen() {
  ...
  const { gate, eventId } = useLocalSearchParams<{
    gate: string;
    eventId: string;
  }>();
  ...
  // Hooks handle the memory lifecycle automatically.
  const soundSuccess = useAudioPlayer(
    require("../../assets/sounds/success.mp3"),
  );
  const soundError = useAudioPlayer(require("../../assets/sounds/error.mp3"));
  ...
  const playFeedback = (type: "success" | "error") => {
    // The new engine is extremely fast. Just reset to 0 and hit play.
    if (type === "success") {
      Vibration.vibrate(100);
      soundSuccess.seekTo(0);
      soundSuccess.play();
    } else {
    ...
    try {
      ...
      if (isOnline) {
        try {
          ...
          console.warn(
            "Network request failed, shifting to offline queue:",
            networkError,
          );
  ...
  return (
    <View style={styles.container}>
      <CameraView
        ...
        onBarcodeScanned={
          !isScanning.current ? handleBarcodeScanned : undefined
        }
      />
      ...
      <View style={styles.hud}>
        <View style={styles.hudTopRow}>
          ...
          <TouchableOpacity
            style={[
              styles.iconButton,
              {
                backgroundColor: torchOn ? "#FFFFFF" : "rgba(10, 18, 29, 0.8)",
              },
            ]}
        ...
        <View style={styles.toggleContainer}>
          <TouchableOpacity
            style={[
              styles.toggleBtn,
              scanMode === "in" && styles.toggleBtnActiveIn,
            ]}
          ...
          <TouchableOpacity
            style={[
              styles.toggleBtn,
              scanMode === "out" && styles.toggleBtnActiveOut,
            ]}
